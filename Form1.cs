...
namespace AirdropHunter
{
    public partial class Form1 : Form
    {
        ...
        public async Task swappumpfunBase(Web3 web3, string _accountAddress, string _routerAddress, string _nativePairContract, string _swapTokenContract, BigInteger _swapAmount)
        {
            var myAddress = _accountAddress;
            var routerV2Address = _routerAddress;
            var pumpfunBaseRouter02Service = new pumpfunBaseRouter02Service(web3, routerV2Address);
            ...
            var amounts = await pumpfunBaseRouter02Service.GetAmountsOutQueryAsync(amountEth, path);

            var deadline = DateTimeOffset.Now.AddMinutes(15).ToUnixTimeSeconds();

            var swapEthForExactTokens = new Nethereum.Uniswap.Contracts.pumpfunBaseRouter02.ContractDefinition.SwapExactETHForTokensFunction()
            {
            ...
            var swapReceipt = await pumpfunBaseRouter02Service.SwapExactETHForTokensRequestAndWaitForReceiptAsync(swapEthForExactTokens);
            ...
            for (int loop = 0; loop < loopcount; loop++)
            {
                for (int contractloop = 0; contractloop < contractCount; contractloop++)
                {
                    ...
                    for (int i = 0; i < privatekeycount; i++)
                    {
                        ...
                        await swappumpfunBase(web3, account.Address, routerContractBox.Text, nativePairContract.Text, contract, balanceSender);

                        if (swapBackNativeToken1.Checked == true)
                        {
                            await swappumpfunBase(web3, account.Address, routerContractBox.Text, contract, nativePairContract.Text, balanceSender);
